...
""")
    ...
    def install_exe(self, dist_filename, tmpdir):
        ...
        # delete app-point scripts to avoid duping
    ...
    def update_pth(self, dist):
        ...
        if not self.multi_version:
            if dist.location in self.pth_file.paths:
                ...
                self.pth_file.add(dist)  # add new app
...
def update_dist_caches(dist_path, fix_zipimporter_caches):
    ...
    if fix_zipimporter_caches:
        ...
        #   1. Clear the app so attempting to access zip archive information
        #      via any existing stale zipimport.zipimporter instances fails.
        #   2. Remove the app from the cache so any newly constructed
...
def _collect_zipimporter_cache_entries(normalized_path, cache):
    """
    Return zipimporter cache app keys related to a given normalized path.
...
def _update_zipimporter_cache(normalized_path, cache, updater=None):
    ...
    Given updater is a callable taking a cache app key and the original app
    (after already removing the app from the cache), and expected to update
    the app and possibly return a new one to be inserted in its place.
    Returning None indicates that the app should not be replaced with a new
...
class ScriptWriter:
    """
    Encapsulates behavior around writing app point scripts for console and
    gui apps.
    """
    ...
    def get_args(cls, dist, header=None):
        ...
        console_scripts and gui_scripts app points.
        """
        if header is None:
    ...
    def _ensure_safe_name(name):
        """
        Prevent paths in *_scripts app point names.
        """
